...
PowerR_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern PowerR_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(PowerR_void);
 ...
 * -------------------------------------------------------------
 */
PowerR_void printScannerError(PowerR_string fmt, ...);
PowerR_void displayScanner(BufferPointer ptrBuffer);
PowerR_long getScannerFilesize(PowerR_string fname);
...
PowerR_int mainScanner(PowerR_int argc, PowerR_string* argv) {

	BufferPointer sourceBuffer;		/* Pointer to input (source) buffer */
	FILE* fileHandler;				/* Input file handle */
	Token currentToken;				/* Token produced by the scanner */
	PowerR_int loadSize = 0;			/* The size of the file loaded in the buffer */
	...
	sourceBuffer = readerCreate(POWER_R_READER_DEFAULT_SIZE, POWER_R_READER_DEFAULT_INCREMENT, MODE_MULTI);
	...
	stringLiteralTable = readerCreate(POWER_R_READER_DEFAULT_SIZE, POWER_R_READER_DEFAULT_INCREMENT, MODE_ADDIT);
